...
export function InsightsPage() {
  ...
  const insights = [
    ...
    ...(worstSla ? [{ icon: '📏', bg: '#3a2a0a', title: 'SLA Risk', text: '<strong style="color:#fbbf24">' + worstSla.agent + '</strong> SLA: <strong style="color:#fbbf24">' + worstSla.slaRate.toFixed(0) + '%</strong>. Target: ' + targets.sla + '%.', explanation: 'SLA % = tickets resolved within SLA deadline / total tickets.', tickets: slaBreachedTickets }] : []),
  ]
  ...
  const recommendations = [
    ...
    ...(m.reopenRate > targets.reopen ? [{ icon: '🔄', bg: '#3a0a0a', title: 'High Reopen Rate', text: 'Reopen rate (' + m.reopenRate.toFixed(1) + '%) exceeds target (' + targets.reopen + '%). Focus on FCR.', explanation: 'A ticket is reopened when status changes from Resolved/Closed back to Open.', tickets: reopenedTickets }] : []),
    { icon: '🌟', bg: '#0a3a1a', title: 'Recognize Top Performer', text: 'Share ' + top.agent + "'s best practices team-wide. Score: " + top.score + '/100.', explanation: 'Recognition boosts team morale. Consider a public shoutout in your team Slack channel.' },
  ]
